    ...
    public int splitArray(int[] nums, int m) {
        ...
        while (start + 1 < end) {
            ...
            if (canSplit(nums, m, mid)) {
                //if we can split the array into <= m arrays wtih curr bound, try a lowerer bound
                end = mid;
            } else {
                start = mid;
                //if we need more subarrays than m arrays with curr bound, we need to increase the bound
            }
        }
